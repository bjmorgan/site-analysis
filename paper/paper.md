...
Molecular dynamics (MD) simulations are widely used to study ionic transport in battery materials, solid electrolytes, and ceramic ion conductors [@LandgrafEtAl_JAmChemSoc2025; @Mercadier2023; @Morgan2021; @PoletayevEtAl_NatureMater2022]. 
...
The package uses a builder pattern interface for configuration and parameter validation. Analyses can be set up and executed with a few lines of code, with results returned as Python lists that integrate simply into downstream analysis workflows. The software operates on lists of pymatgen `Structure` objects, making it compatible with any trajectory source that can produce pymatgen structures.
