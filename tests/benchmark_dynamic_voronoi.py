...
Compares total wall-clock time for per-site vs batch-optimised paths
and optionally dumps a cProfile breakdown with ``--profile``.
...
def _per_site_analyse_structure(self, atoms, structure):
    """Per-site analysis without batch optimisation.

    Calls calculate_centre on each site individually, performing
...
def main():
    ...
    if args.profile:
        ...
        # Per-site (no batch caching) path
        print("\nPer-site (no batch optimisation):")
        ...
        # Summary
        print("\n--- Summary ---")
        print(f"  Per-site:  {elapsed_orig:.2f}s  ({elapsed_orig/n*1000:.1f}ms/frame)")
        print(f"  Batch:     {elapsed_opt:.2f}s  ({elapsed_opt/n*1000:.1f}ms/frame)")
