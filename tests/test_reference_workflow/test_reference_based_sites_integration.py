...
class TestCoordinationValidation(unittest.TestCase):
    ...
    def test_octahedral_coordination_in_small_cell(self):
        """Test that a small unit cell with too few unique coordinating atoms
        yields no matching environments when requesting octahedral coordination.

        With minimum-image distances each atom pair contributes only one
        distance, so the 3 unique Cl atoms cannot satisfy n_vertices=6.
        No environments are found, and therefore no sites are created.
        """
        lattice = Lattice.cubic(5.0)
        structure = Structure(
            lattice=lattice,
            ...
            coords=[
                ...
                [0.5, 0.0, 0.0],      # Cl at x face centre
                [0.0, 0.5, 0.0],      # Cl at y face centre
                [0.0, 0.0, 0.5],      # Cl at z face centre
            ],
        )

        rbs = ReferenceBasedSites(structure, structure.copy(), align=False)

        # Only 3 unique Cl atoms exist; requesting 6 returns no environments
        sites = rbs.create_polyhedral_sites(
            center_species="Na",
            vertex_species="Cl",
            cutoff=3.0,
