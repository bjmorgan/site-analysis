...
class GetCoordinationIndicesTestCase(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test arrays with varying coordination environments."""
        # 3 Na atoms with different Cl coordination, lattice constant 10.0
        self.test_lattice_matrix = np.eye(3) * 10.0
        self.test_species = ["Na", "Na", "Na", "Cl", "Cl", "Cl", "Cl", "Cl", "Cl", "Cl", "Cl", "Cl", "Cl"]
        self.test_frac_coords = np.array([
            [0.0, 0.0, 0.0],    # Na1 (idx 0): 4 Cl neighbours within 2.0 A
            [0.5, 0.5, 0.5],    # Na2 (idx 1): 6 Cl neighbours within 2.0 A
            [0.8, 0.8, 0.8],    # Na3 (idx 2): 0 Cl neighbours within 2.0 A
        ...
        # Multi-species arrays for coordination
        self.multi_species_lattice_matrix = np.eye(3) * 5.0
        self.multi_species_species = ["Mg", "O", "O", "F", "F"]
        self.multi_species_frac_coords = np.array([
            [0.0, 0.0, 0.0],  # Mg (idx 0) with 2 O and 2 F neighbours
        ...
        # Distance gradient arrays
        self.distance_lattice_matrix = np.eye(3) * 10.0
        self.distance_species = ["Na", "Cl", "Cl", "Cl", "Cl", "Cl"]
        self.distance_frac_coords = np.array([
            [0.0, 0.0, 0.0],  # Na
            [0.1, 0.0, 0.0],  # Cl - 1.0 A away
            [0.0, 0.2, 0.0],  # Cl - 2.0 A away
            [0.0, 0.0, 0.3],  # Cl - 3.0 A away
            [0.4, 0.0, 0.0],  # Cl - 4.0 A away
            [0.0, 0.5, 0.0],  # Cl - 5.0 A away
        ])

    def test_exact_coordination_matching(self):
        """Test that only environments with exactly matching coordination are returned."""
        # Test exact match for environments with 4 Cl neighbours
        ...
        # Test exact match for environments with 6 Cl neighbours
        ...
        # Test no environments with 5 Cl neighbours
    ...
    def test_multiple_species(self):
        ...
        environments = get_coordination_indices(
            frac_coords=self.multi_species_frac_coords,
            lattice_matrix=self.multi_species_lattice_matrix,
            species=self.multi_species_species,
            centre_species="Mg",
    ...
    def test_variable_coordination(self):
        """Test with different coordination numbers for different centres."""
        # Request customised coordination for each Na atom
        environments = get_coordination_indices(
            frac_coords=self.test_frac_coords,
            lattice_matrix=self.test_lattice_matrix,
            species=self.test_species,
        ...
        environments = get_coordination_indices(
            ...
            coordination_species="Cl",
            cutoff=2.0,
            n_coord=[5, 5, 5],  # No Na has exactly 5 Cl neighbours
        )
    ...
    def test_edge_cases_and_errors(self):
        ...
        # Should return environments with 0 neighbours (for all Na)
        self.assertEqual(len(environments), 3)
    ...
    def test_list_of_coordination_species(self):
        ...
        environments = get_coordination_indices(
            ...
            species=self.test_species,
            centre_species="Na",
            coordination_species=["Cl", "K"],  # K doesn't exist
            cutoff=2.7,
            n_coord=4,
        )
        ...
        environments = get_coordination_indices(
            ...
            species=self.test_species,
            centre_species="Na",
            coordination_species=["K", "Ca"],  # Neither exists
            cutoff=2.7,
            n_coord=0,
        )
    ...
    def test_cutoff_sensitivity(self):
        ...
        environments = get_coordination_indices(
            frac_coords=self.distance_frac_coords,
            lattice_matrix=self.distance_lattice_matrix,
            species=self.distance_species,
            centre_species="Na",
            coordination_species="Cl",
            cutoff=1.5,
            n_coord=1,
        ...
        # With 2.5 A cutoff, should find exactly 2 neighbours
        environments = get_coordination_indices(
            frac_coords=self.distance_frac_coords,
            lattice_matrix=self.distance_lattice_matrix,
            species=self.distance_species,
        ...
        self.assertEqual(len(environments), 1)
        self.assertEqual(len(environments[0]), 2)
        self.assertEqual(set(environments[0]), {1, 2})

        # With 3.5 A cutoff, should find exactly 3 neighbours
        environments = get_coordination_indices(
            frac_coords=self.distance_frac_coords,
            lattice_matrix=self.distance_lattice_matrix,
            species=self.distance_species,
        ...
        self.assertEqual(set(environments[0]), {1, 2, 3})

        # With 3.5 A cutoff but requesting 2 neighbours, should return nothing
        environments = get_coordination_indices(
            frac_coords=self.distance_frac_coords,
            lattice_matrix=self.distance_lattice_matrix,
            species=self.distance_species,
        ...
        # With 3.5 A cutoff but requesting 4 neighbours, should return nothing
        environments = get_coordination_indices(
            frac_coords=self.distance_frac_coords,
            lattice_matrix=self.distance_lattice_matrix,
            species=self.distance_species,
